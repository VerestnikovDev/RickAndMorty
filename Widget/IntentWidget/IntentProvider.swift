//
//  IntentProvider.swift
//  MyWidgetExtension
//
//  Created by a.verestnikov on 31.03.2021.
//

import WidgetKit

struct IntentProvider: IntentTimelineProvider {

    typealias Entry = EntryData
    typealias Intent = ConfigurationIntent

    func placeholder(in context: Context) -> EntryData {
        .defaultValue
    }

    func getSnapshot(for configuration: ConfigurationIntent, in context: Context, completion: @escaping (EntryData) -> Void) {
        completion(.defaultValue)
    }

    func getTimeline(for configuration: ConfigurationIntent, in context: Context, completion: @escaping (Timeline<EntryData>) -> Void) {
        var name: String?
        switch configuration.source {
        case .app:
            let storage = UserDefaults(suiteName: "group.com.verestnikov.dev.RickAndMorty")
            name = storage?.string(forKey: "CharacterName") ?? "Rick"
        case .widget:
            name = configuration.name
        case .unknown:
            break
        }

        CharacterLoader.load(name: name ?? "Rick") { result in
            let entry: EntryData
            switch result {
            case let .success(character):
                let name = character?.name ?? "Rick"
                let status = character?.status ?? .unknown
                let url = character?.image ?? ""
                entry = .init(date: Date(), name: name, status: status, url: url)
            case .failure:
                entry = .defaultValue
            }
            let timeline = Timeline(entries: [entry], policy: .never)
            completion(timeline)
        }
    }

}
